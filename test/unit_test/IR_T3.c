 ...
 * @details
 *   - Uses mock ADC value for the line sensor.
 *   - Verifies ir_line_detected() behaviour on:
...
/* --------------------------------------------------------------------------
 * Mocks and Stubs for Hardware (analog line only)
 * -------------------------------------------------------------------------- */
...
uint16_t adc_read(void) { return 0; }   // not used directly

// Override line sensor read to use our mock value
uint16_t ir_read_line_sensor(void) { return mock_line_adc; }

// --- Minimal GPIO/time stubs to satisfy ir_sensor.c ---
...
int main(void) {
    ...
    // Use calibration values from ir_sensor.c
    ...
    // Expect: far from threshold on white side → not considered "on edge"
    mock_line_adc = white;
    ASSERT_FALSE(ir_line_detected(), "Line sensor on WHITE: ir_line_detected() == false");

    // Case 2: PURE BLACK line
    // Typically you only consider "on edge" when near threshold; pure black
    // should not be treated as edge either.
    mock_line_adc = black;
    ...
    // Here we expect ir_line_detected() to be true.
    mock_line_adc = thr;
