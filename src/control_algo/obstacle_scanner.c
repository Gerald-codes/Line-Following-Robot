/**
 * @file    obstacle_scanner.c
 * @brief   Obstacle scanning and detection implementation
 * @details Implements servo-based ultrasonic scanning with signal smoothing,
 *          obstacle detection, width calculation, and avoidance direction
 *          determination with telemetry support
 */
...
/**
 * @brief Get smoothed distance by averaging last readings
 * @param new_distance New distance measurement
 * @return Smoothed distance value
 */
...
/**
 * @brief Calculate obstacle width based on angle span and distance
...
/**
 * @brief Perform complete scan sweep
 * @return ScanResult containing all detected obstacles
 */
ScanResult scanner_perform_scan(void)
{
    ...
    for (int angle = MIN_ANGLE; angle <= MAX_ANGLE; angle += SCAN_STEP)
    {
        ...
        if (status == SUCCESS)
        {
            ...
            printf("Angle: %3d° | Distance: %3llu cm (smoothed: %3llu cm)", 
                   angle, distance, smoothed_distance);
 ...
 * @param result ScanResult to display
 */
...
/**
 * @brief Analyze clear space on each side
 * @param result ScanResult from scan
 * @param left_clear Output: degrees of clear space on left
 * @param right_clear Output: degrees of clear space on right
 */
void scanner_get_clear_space_analysis(ScanResult result, int* left_clear, int* right_clear)
{
    ...
    *right_clear = 0;
    ...
    /* Create boolean array to mark blocked positions */
    ...
    /* Count clear space on RIGHT side (servo mounted backwards) */
    ...
    /* Count clear space on LEFT side (servo mounted backwards) */
...
/**
 * @brief Determine best avoidance direction based on scan
 * @param result ScanResult from scan
 * @return AvoidanceDirection (LEFT, RIGHT, or NONE)
 */
AvoidanceDirection scanner_get_best_avoidance_direction(ScanResult result)
{
    ...
    else
    {
        ...
        /* Find where the obstacle is closest */
        uint64_t min_distance_overall = 999;
        int min_distance_angle = ANGLE_CENTER;
        
        /* Analyze distance at multiple points */
        ...
        for (int i = 0; i < result.obstacle_count; i++)
        {
            ...
            for (int angle = obs->angle_start; angle <= obs->angle_end; angle += SCAN_STEP)
            {
                ...
                if (dist_index >= 0 && dist_index < 41 && result.distances[dist_index] > 0)
                {
                    ...
                    /* Accumulate distances (SWAPPED for servo orientation) */
                    if (angle < ANGLE_CENTER)
                    {
                        right_total_distance += dist;
                        right_count++;
                    }
                    else
                    {
                        left_total_distance += dist;
                        left_count++;
                    }
                }
            }
        ...
        /* Decision: go away from closest point */
