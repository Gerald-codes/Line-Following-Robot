 ...
 * @brief   Interrupt-based Code 39 barcode scanner module
 * @details
...
/**
 * @brief   Barcode command type - represents the decoded direction
 */
typedef enum {
...
/**
 * @brief   Scanner state - for external monitoring if needed
 */
typedef enum {
 ...
 * @details Sets up GPIO interrupt on the barcode sensor pin.
...
/**
 * @brief   Update the barcode scanner (call in main loop)
 * @details Checks for scan completion/timeout and decodes barcodes.
 * @return  BarcodeCommand - the decoded direction or NONE
 */
BarcodeCommand barcode_scanner_update(void);

/**
 * @brief   Reset the scanner state
 * @details Clears any in-progress scans and resets to IDLE.
 ...
 * @details This is useful for handling numbers and special characters
...
/**
 * @brief   Check if scanner is ready for a new scan
 * @return  true if in IDLE state, false otherwise
 */
bool barcode_scanner_is_ready(void);

/**
 * @brief   Convert BarcodeCommand to string (for debugging)
